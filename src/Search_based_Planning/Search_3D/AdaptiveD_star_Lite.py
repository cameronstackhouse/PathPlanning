...
class TreeNode:
    ...
    def is_uniform(self):
        ...
        print(len(self.env.blocks))
        for block in self.env.blocks:
            if (self.x < block[3] and self.x + self.width > block[0] and
                self.y < block[4] and self.y + self.height > block[1] and
                self.z < block[5] and self.z + self.depth > block[2]):
                return False
        return True
    ...
    def partition(self, leafs):
        ...
        if self.is_leaf() and (
            #not self.is_uniform()
            self.contains_point(self.env.start)
            ...
            self.one = TreeNode(self.x, self.y, self.z, mid_width, mid_height, mid_depth, self.env)
            self.two = TreeNode(self.x + mid_width, self.y, self.z, self.width - mid_width, mid_height, mid_depth, self.env)
            self.three = TreeNode(self.x, self.y + mid_height, self.z, mid_width, self.height - mid_height, mid_depth, self.env)
            self.four = TreeNode(self.x + mid_width, self.y + mid_height, self.z, self.width - mid_width, self.height - mid_height, mid_depth, self.env)
            self.five = TreeNode(self.x, self.y, self.z + mid_depth, mid_width, mid_height, self.depth - mid_depth, self.env)
            self.six = TreeNode(self.x + mid_width, self.y, self.z + mid_depth, self.width - mid_width, mid_height, self.depth - mid_depth, self.env)
            self.seven = TreeNode(self.x, self.y + mid_height, self.z + mid_depth, mid_width, self.height - mid_height, self.depth - mid_depth, self.env)
            self.eight = TreeNode(self.x + mid_width, self.y + mid_height, self.z + mid_depth, self.width - mid_width, self.height - mid_height, self.depth - mid_depth, self.env)
            ...
            leafs.remove(self)
            leafs.extend([self.one, self.two, self.three, self.four, self.five, self.six, self.seven, self.eight])
...
class Octree:
    ...
    def _draw_cube(self, ax, node):
        ...
        ax.add_collection3d(Poly3DCollection(faces, facecolors='white', linewidths=1, edgecolors='r', alpha=.25))
...
if __name__ == "__main__":
    plan = ADStarLite(1)
    plan.change_env("Evaluation/Maps/3D/block_map_25_3d/12_3d.json")
    print(plan.env.blocks)
